...
export function AdminPostsTable({ posts }: { posts: AdminPost[] }) {
  ...
  function executeDelete(id: string) {
    ...
    startTransition(async () => {
      await adminDeletePost(id, reason);
      setDeleting(null);
      setReason("");
    });
  }
      ...
      {posts.map(post => (
        <div key={post.id} className="p-4">
          {editing === post.id ? (
            ...
            <div className="space-y-2">
              <p className="text-xs text-black/50 line-clamp-2 italic">&ldquo;{post.content}&rdquo;</p>
              <input
                autoFocus
                value={reason}
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                <button onClick={() => confirmDelete(post.id)} disabled={isPending}
