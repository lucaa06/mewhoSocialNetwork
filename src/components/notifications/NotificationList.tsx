...
function NotificationItem({ n }: { n: Notification }) {
  ...
  if (n.type === "follow") {
    text = (
      <>
        <strong>{actorName}</strong> ha iniziato a seguirti
      </>
    );
    href = `/u/${actorUser}`;
    Icon = UserPlus;
    ...
    if (communityName) {
      text = (
        <>
          <strong>{actorName}</strong> ha pubblicato nella community{" "}
      ...
      text = (
        <>
          <strong>{actorName}</strong> ha pubblicato un nuovo post
          {postTitle ? `: "${postTitle}"` : ""}
        </>
      );
    }
    ...
    text = (
      <>
        <strong>{actorName}</strong> ha reagito {emoji} al tuo post
        {postTitle ? ` "${postTitle}"` : ""}
      </>
    );
    ...
    text = (
      <>
        <strong>{actorName}</strong> ha commentato
        {postTitle ? ` "${postTitle}"` : " il tuo post"}
        {preview ? `: "${preview}"` : ""}
      </>
    );
    href = p.post_id ? `/post/${p.post_id}` : "/";
    ...
    text = (
      <>
        <strong>{actorName}</strong> ti ha menzionato
        {postTitle ? ` in "${postTitle}"` : ""}
      </>
    );
    href = p.post_id ? `/post/${p.post_id}` : "/";
    Icon = AtSign;
  ...
  if (notifications.length === 0) {
    return (
      ...
      >
        ...
        <p className="text-xs mt-1" style={{ color: "var(--subtle)" }}>
          Ti avviseremo quando qualcuno interagisce con te
        </p>
      </div>
    );
  }
